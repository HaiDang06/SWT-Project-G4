...
public class StaffDAO {
    ...
    private Staff extractStaffFromResultSet(ResultSet rs) throws SQLException {
        ...
        staff.setPassword(rs.getString("Password")); // Cần thiết để giữ lại pass khi update
        staff.setIsActive(rs.getBoolean("IsActive"));

        // FR-07: Ánh xạ 5 trường mới
        if (rs.getDate("Dob") != null) staff.setDob(rs.getDate("Dob").toLocalDate());
        staff.setDepartment(rs.getString("Department"));
        staff.setPosition(rs.getString("Position"));
        staff.setSalary(rs.getDouble("Salary"));
        if (rs.getDate("HireDate") != null) staff.setHireDate(rs.getDate("HireDate").toLocalDate());
    ...
    /**
     * Lấy thông tin Staff thông qua Email để phục vụ đăng nhập
     */
    ...
    public int countStaffByFilter(String searchId, String searchName, String searchDept) {
        ...
        if (searchName != null && !searchName.trim().isEmpty()) sql.append(" AND s.FullName ILIKE ?"); // Dùng ILIKE cho PostgreSQL (không phân biệt hoa thường)
